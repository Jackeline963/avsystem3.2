...
public function get_productos_vendidos($sucursal,$desde){
    ...
    $sql="select p.id_producto,p.marca,p.modelo,p.color,p.medidas,p.diseno,p.materiales,p.categoria,dv.fecha_venta,dv.numero_venta,v.sucursal,count(p.id_producto) as cantidad_vendidos from productos as p inner join detalle_ventas as dv on p.id_producto=dv.id_producto INNER join ventas as v on dv.numero_venta=v.numero_venta WHERE v.sucursal LIKE '%Metrocentro%' and v.fecha_venta like '%01-2022%' and p.categoria_producto='aros' group by p.id_producto order by dv.fecha_venta DESC;";
    $sql=$conectar->prepare($sql);
    $sql->bindValue(1,$suc);
